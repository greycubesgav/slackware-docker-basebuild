...
docker-image-build:
	...
	docker build --build-arg BASE_IMAGE="$(BASE_IMAGE)/slackware" --build-arg BASE_VERSION=$(BASE_VERSION) --platform $(DOCKER_PLATFORM) --file Dockerfile --tag $(DOCKER_FULL_IMAGE_NAME) .
